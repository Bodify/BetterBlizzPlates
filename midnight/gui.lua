...
StaticPopupDialogs["BBP_CONFIRM_WIPE_NPCCOLOR"] = {
    text = titleText.. BBP.L("This will delete the entire npc color list and reload.\n\nAre you sure?"),
    button1 = BBP.L("Yes"),
    button2 = BBP.L("No"),
    OnAccept = function()
...
StaticPopupDialogs["BBP_CONFIRM_IMPORT_NPCCOLOR"] = {
    text = titleText.. BBP.L("This will add Mythic+ Season 3 NPCs to your color list and reload.\n\nAre you sure?"),
    button1 = BBP.L("Yes"),
    button2 = BBP.L("No"),
    OnAccept = function()
...
StaticPopupDialogs["BBP_CONFIRM_WIPE_CASTEMPHASIS"] = {
    text = titleText.. BBP.L("This will delete the entire cast list and reload.\n\nAre you sure?"),
    button1 = BBP.L("Yes"),
    button2 = BBP.L("No"),
    OnAccept = function()
...
StaticPopupDialogs["BBP_TOTEMLIST_RESET"] = {
    text = titleText.. BBP.L("This will delete the entire totem list and reset it back to its default state.\nA reload will be neccesary.\n\nAre you sure you want to continue?"),
    button1 = BBP.L("Yes"),
    button2 = BBP.L("No"),
    OnAccept = function()
        BBP.ResetTotemList()
...
local function CreateTextureDropdown(name, parentFrame, labelText, settingKey, toggleFunc, point, dropdownWidth, maxVisibleItems)
    ...
    dropdown:SetPoint("BOTTOMLEFT", container, "BOTTOMLEFT", 0, 0)
    dropdown:SetWidth(dropdownWidth or 155)
    dropdown:SetDefaultText(BBP.L(BetterBlizzPlatesDB[settingKey] or "Select texture"))
    dropdown.Background:SetVertexColor(0.9,0.9,0.9)
    dropdown.Arrow:SetVertexColor(0.9,0.9,0.9)
...
local function CreateTooltipTwo(widget, title, mainText, subText, anchor, cvarName, cvarName2, category)
    ...
    widget:SetScript("OnEnter", function(self)
        ...
        if widget == BBP.friendlyHideHealthBar then
            ...
            local tooltipText = BBP.L("|cff32f795Right-click to keep them enabled on your Target.|r")
        ...
        elseif originalTitle == "Hide NPC Healthbar" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Hide Enemy Castbar" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Hide Cooldown Text on Debuffs" then
        ...
        elseif originalTitle == "Hide Castbar Text" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Hide Friendly Castbar" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Hide Level" and BetterBlizzPlatesDB.classicNameplates then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Color Focus Nameplate Healthbar" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Stacking Nameplates" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Purgeable" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Hide Enemy Name" then
        ...
        elseif originalTitle == "Sort Auras by Duration" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Enable Key Auras" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Castbar Background Color" then
        ...
        elseif originalTitle == "Center Auras on Enemy" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Nameplate Resource" then
            ...
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        elseif originalTitle == "Friend/Guildie Indicator" then
            local currentAnchor = BetterBlizzPlatesDB.friendIndicatorAnchor or "LEFT"
            local tooltipText = BBP.L("\n|cff32f795Right-click to change anchor: ") .. BBP.L(currentAnchor) .. "|r"
            GameTooltip:AddLine(tooltipText, 1, 1, 1, true)
        end
...
local function ShowProfileConfirmation(profileName, class, profileFunction, additionalNote)
    ...
    local profileText = string.format("|A:%s:16:16|a %s%s|r", icon, color, profileName.. BBP.L(" Profile"))
    local confirmationText = titleText .. BBP.L("This action will delete all settings and apply\nthe ") .. profileText .. BBP.L(" and reload the UI.\n\n") .. noteText .. BBP.L("Are you sure you want to continue?")
...
local function CreateClassButton(parent, class, name, twitchName, onClickFunc, youtubeName)
    ...
    if class == "STARTER" then
        CreateTooltipTwo(button, string.format("|A:%s:16:16|a %s%s|r", icon, color, name.. BBP.L(" Profile")), "A basic starter profile that only enables the few things you need.\n\nIntended to work as a very minimal quick start that can be built upon.", nil, "ANCHOR_TOP")
    elseif class == "BLITZ" then
        CreateTooltipTwo(button, string.format("|A:%s:16:16|a %s%s|r", icon, color, name.. BBP.L(" Profile")), "A more advanced profile enabling a few more settings and customizing things a bit more.\n\nGreat for Battlegrounds (and Arenas) with Class Icons showing Healers, Tanks and Battleground Objectives.", nil, "ANCHOR_TOP")
    elseif class == "MYTHIC" then
        CreateTooltipTwo(button, string.format("|A:%s:16:16|a %s%s|r", icon, color, name.. BBP.L(" Profile")), "A tweaked version of the Midnight nameplates with NPC colors enabled.", nil, "ANCHOR_TOP")
    elseif name == "Bodify" then
        CreateTooltipTwo(button, string.format("|A:%s:16:16|a %s%s|r", icon, color, name.. BBP.L(" Profile")), "My personal profile from a while ago. Meant for Arenas only. Possible I'd make some tweaks if I was actively playing still.", nil, "ANCHOR_TOP")
    elseif name == "Pre-Midnight" then
        CreateTooltipTwo(button, string.format("|A:%s:16:16|a %s%s|r", icon, color, name.. BBP.L(" Profile")), "A very basic profile that aims to be similar to how the nameplates looked like before Midnight. A few adjustments that can be tuned later on.", nil, "ANCHOR_TOP")
    else
        local socialText = ""
        if twitchName then
        ...
        CreateTooltipTwo(button, string.format("|A:%s:16:16|a %s%s|r", icon, color, name.. BBP.L(" Profile")), string.format("Enable all of %s's profile settings.", name), socialText ~= "" and socialText or nil, "ANCHOR_TOP")
    end
...
local function CreateImportExportUI(parent, title, dataTable, posX, posY, tableName)
    ...
    importBtn:SetScript("OnClick", function()
        if InCombatLockdown() then
            print(BBP.L("|A:gmchat-icon-blizz:16:16|aBetter|cff00c0ffBlizz|rPlates: Leave combat to Import"))
            return
        end
...
local function guiProfiles()
    ...
    frame.titleText:SetText(BBP.L("|A:gmchat-icon-blizz:16:16|a BBP"))
    ...
    frame.descriptionText:SetText(BBP.L("Profiles for BetterBlizzPlates:"))
    ...
    frame.coreText:SetText(BBP.L("Core"))
    ...
    frame.streamerText:SetText(BBP.L("Streamers"))
...
local function guiGeneralTab()
    ...
    midnightBeta:SetText(BBP.L("|T136221:12:12|t |cffcc66ffBetterBlizzPlates EARLY BETA (Beware of bugs! WIP)."))
    ...
    local function OpenSmallPetsOptionsWindow()
        if not smallPetsOptionsFrame then
            ...
            smallPetsOptionsFrame.title:SetText(BBP.L("Small Pets Options"))
    ...
    nameplateGeneralHpHeightResetButton:SetText(BBP.L("Default"))
...
--[[
    ...
    classIndicator:HookScript("OnClick", function(self)
        ...
        if self:GetChecked() then
            if not BetterBlizzPlatesDB.enableNameplateAuraCustomisation then
                print(BBP.L("|A:gmchat-icon-blizz:16:16|aBetter|cff00c0ffBlizz|rPlates: Enable Nameplate Aura customization in order to show CC icons in Class Indicator."))
            else
                print(BBP.L("|A:gmchat-icon-blizz:16:16|aBetter|cff00c0ffBlizz|rPlates: Enabled Friendly Debuffs & PvP CC filter in Nameplate Auras section in order to show CC inside of Class Indicator."))
            end
    ...
    partyPointer:HookScript("OnClick", function(self)
        if self:GetChecked() then
            if not BetterBlizzPlatesDB.enableNameplateAuraCustomisation then
                print(BBP.L("|A:gmchat-icon-blizz:16:16|aBetter|cff00c0ffBlizz|rPlates: Enable Nameplate Aura customization in order to show CC icons on top of Party Pointer."))
            else
                print(BBP.L("|A:gmchat-icon-blizz:16:16|aBetter|cff00c0ffBlizz|rPlates: Enabled Friendly Debuffs & PvP CC filter in Nameplate Auras section in order to show CC on top of Party Pointer."))
            end
            if BBP.friendlyNpdeBuffEnable then
                if not BBP.friendlyNpdeBuffEnable:GetChecked() then
                    BBP.friendlyNpdeBuffEnable:Click()
        ...
        if button == "RightButton" then
            if IsShiftKeyDown() then
                ...
                RefreshTooltip(enableCustomFontOutline, "Font Outline",
                    "Enable font outline.\n|cff32f795Right-click to swap between thick and thin outline.\nCurrent: " ..
                    (BetterBlizzPlatesDB["customFontOutline"] == "THICKOUTLINE" and "Thick Outline" or "Thin Outline") .. BBP.L("\n|cff87ceebShift+Right-click to toggle font shadow.\nCurrent: ") .. shadowState)
            else
    ...
    arenaSettingsText:SetText(BBP.L("Arena Names"))
    ...
    resetBBPButton:SetText(BBP.L("Full Reset"))
...
local function guiPositionAndScale()
    ...
    -- anchorSubPet.petHealthColor:SetText(BBP.L("Color"))
    ...
    anchorSubFaction:SetText(BBP.L("Faction Indicator"))
    ...
    anchorSubFaction.factionIconSetLabel:SetText(BBP.L("Icon Set"))
    ...
    anchorSubArena:SetText(BBP.L("Arena Names"))
    ...
    anchorSubClassIcon.classIndicatorCCAuras:HookScript("OnClick", function(self)
        if self:GetChecked() then
            print(BBP.L("|A:gmchat-icon-blizz:16:16|aBetter|cff00c0ffBlizz|rPlates: Enabled Friendly Debuffs & PvP CC filter in Nameplate Auras section."))
            if BBP.friendlyNpdeBuffEnable then
                if not BBP.friendlyNpdeBuffEnable:GetChecked() then
                    BBP.friendlyNpdeBuffEnable:Click()
                end
    ...
    anchorSubClassIcon.classIndicatorShowPet:HookScript("OnClick", function(self)
        if self:GetChecked() then
            BBP.RunAfterCombat(function()
                ...
                print(BBP.L("|A:gmchat-icon-blizz:16:16|a Better|cff00c0ffBlizz|rPlates: Show Friendly Pets CVar toggled on for Pet Class Indicator to work. If you want to turn this back off go to /bbp -> CVar Control and uncheck it."))
            end)
        end
    end)
    ...
    anchorSubClassIcon.extendedSettingsButton:SetText(BBP.L("More options"))
    ...
    anchorSubClassIcon.extendedSettings.name = BBP.L("Advanced Settings")
    anchorSubClassIcon.extendedSettings:SetTitle(BBP.L("Class Indicator"))
    ...
    anchorSubHealthNumbers.extendedSettings:SetTitle(BBP.L("Health Numbers"))
    ...
    anchorSubTargetText:SetText(BBP.L("Target Text"))
    ...
    anchorSubBlitzIndicator:SetText(BBP.L("Blitz Indicator"))
    ...
    contentFrame.anchorSubTotem:SetText(BBP.L("Totem Indicator"))
...
--[=[
    ...
    BetterBlizzPlatesSubPanel.reloadButton:SetText(BBP.L("Reload UI"))
    ...
    BetterBlizzPlatesSubPanel.resetBBPButton:SetText(BBP.L("Reset BetterBlizzPlates"))
    ...
    BetterBlizzPlatesSubPanel.rightClickTip:SetText(BBP.L("|A:smallquestbang:20:20|aTip:  Right-click sliders to enter a specific value"))
end
...
local function guiCastbar()
    ...
    -- castEmphasisText:SetText(BBP.L("Cast Emphasis List"))
...
local function guiClickingAndStacking()
    ...
    guiClickNStack.name = BBP.L("|A:plunderstorm-pickup-mouseclick-left:16:16|aLook & Behaviour")
    ...
    settingsText:SetText(BBP.L("General"))
    ...
    stackingText:SetText(BBP.L("Stacking"))
    local icon = guiClickNStack:CreateTexture(nil, "ARTWORK")
    ...
    clickingText:SetText(BBP.L("Clicking"))
    local icon = guiClickNStack:CreateTexture(nil, "ARTWORK")
    ...
    castbarText:SetText(BBP.L("Castbar Adjustments"))
    local icon = guiClickNStack:CreateTexture(nil, "ARTWORK")
    ...
    clickAndStackTestButton:SetText(BBP.L("Test"))
    ...
    clickAndStackTestButton:SetScript("OnClick", function(self)
        ...
        if testModeActive then
            self:SetText(BBP.L("Stop Testing"))
            BBP.ClickAndStackTestMode(true)
        else
            self:SetText(BBP.L("Test"))
...
local function guiHideCastbar()
    ...
    hideCastbarExplanationText:SetText(BBP.L("Hide castbar for chosen spells/NPCs,\nor only show whitelisted ones.\n \nSupports spell name/id and npc id/name"))
...
local function guiColorNPC()
    ...
    npcColorSettingsText:SetText(BBP.L("NPC Color settings"))
...
local function guiAuraColor()
    ...
    prioText:SetText(BBP.L("Priority Value"))
...
--             ccOptionsFrame.title:SetText(BBP.L("PvP CC (All units)"))
...
--             buffOptionsFrame.title:SetText(BBP.L("PvP Buffs (All units)"))
...
--                 -- Tooltip
--                 CreateTooltipTwo(glowCheckbox, "Important Glow |A:importantavailablequesticon:22:22|a", "Check for a glow on all "..buffFilter.tt.. BBP.L("\n\n|cff32f795Right-click to change Color.|r"), "Auras that are in the whitelist with glow enabled will override this behavior", "ANCHOR_TOPRIGHT")
...
--     pvpCC.Text:SetText(BBP.L("Glow & Filter"))
...
--     prdText:SetText(BBP.L("Personal Resource Display Specific:"))
...
local function guiCVarControl()
    ...
    local function OpenClassSpecificWindow()
        if not classOptionsFrame then
            ...
            classOptionsFrame.title:SetText(BBP.L("Class Specific Options"))
            ...
            for i, classData in ipairs(classes) do
                ...
                local className = LOCALIZED_CLASS_NAMES_MALE[string.upper(classData.class)] or classData.class
                classCheckbox.Text:SetText(BBP.L("Ignore") .. " " .. className)
...
local function guiImportAndExport()
    ...
    profilesBtn:SetText(BBP.L("PROFILES SELECTION"))
...
local function guiTemp()
    ...
    guiTemp.name = BBP.L("|cff00ff00NEW SETTINGS (TEMP)")
    ...
    settingsText:SetText(BBP.L("Temporary space for new Midnight related settings. Still a lot of work left."))
